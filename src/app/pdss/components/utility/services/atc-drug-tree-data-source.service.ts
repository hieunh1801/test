...
export class AtcDrugTreeDataSourceService {
  ...
  initTree(): void {
    this.getAtcCode().subscribe((nodeList) => {
        ...
        .subscribe((response) => {
          ...
          const nodeList: DrugTreeNode[] = drugList.map((row) => {
            return {
              ...
              drugHasPharmgkbDruglabel: row.hasPharmgkbDruglabel,
              drugHasPharmgkbGuideline: row.hasPharmgkbGuideline,
              drugHasPharmgkbVa: row.hasPharmgkbVa,
              drugHasRxAnnotation: row.hasRxAnnotation,
              drugHasSpmedGuideline: row.hasSpmedGuideline,
            };
          });
  ...
  private getDrugByAtcCode(drugName: string, atcCodeLevel4: string): any {
    const pageIndex = 0;
    const pageSize = 10000;
    const searchRequest: AtcDrugSearchRequest = {
